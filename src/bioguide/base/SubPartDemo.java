...
import y.view.hierarchy.GroupLayoutConfigurator;
  ...
  *  and edges can be set (see {@link #configureDefaultRealizers()}).
   ...
   * </p>
   */
  public class SubPartDemo extends JPanel implements Graph2DSelectionListener
  {
    Graph2DView view;
          ...
          //  }
           ...
           // view.setFitContentOnResize(true);
            ...
            graph.addGraph2DSelectionListener(this);
             ...
             BufferedReader in = new BufferedReader(new FileReader("aa.sif"));
             String line;
             String []nodelist = new String[500];
             int index = 0;
             ...
             while((line = in.readLine()) != null)
             {
                 ...
                 {
                     if(nodelist[i].equals(subs[0]))
                         pre1 = true;
                     if(nodelist[i].equals(subs[2]))
                             pre2 = true;

                 }
                 if(!pre1)
                 {
                     nodelist[index++] = subs[0];
                 }
                 if(!pre2)
                 {
                     nodelist[index++] = subs[2];
                 }
             ...
             for(int i = 0; i < index; i++)
            {
                  if((i % 40) == 0)
                  {
                    ...
                    nodes[i] = graph.createNode(column*50,row*50, nodelist[i]);
                    column++;
              //else nodes[i] = graph.createNode(i*50, i*50, nodelist[i]);
            }

             BufferedReader comb = new BufferedReader(new FileReader("aa.sif"));

             while((line = comb.readLine()) != null)
             {
                 line = line.trim();
                 String [] subs = line.split(" ");
                 ...
                 graph.createEdge(nodes[src], nodes[tgt]);
...
OrganicLayouter hgl = new OrganicLayouter();
...
//glc.restoreAll();
            ...
            Logger.getLogger(SubPartDemo.class.getName()).log(Level.SEVERE, null, ex);
        }
